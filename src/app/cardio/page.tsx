...
export default function CardioPage() {
  ...
  // Progress circle
  const totalForStage = stage === 'hiit-work' ? config.workSeconds
    : stage === 'hiit-rest' ? config.restSeconds
    : stage === 'liss' ? totalLiss : 1
  ...
  if (!started) {
    return (
      <div className="page fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100dvh', textAlign: 'center' }}>
        ...
        <button className="btn-primary" onClick={() => setStarted(true)} style={{ fontSize: 18, padding: '16px 40px' }}>
          EMPEZAR
        </button>
        <button onClick={() => router.push('/home')} style={{ background: 'none', border: 'none', color: 'var(--muted)', fontSize: 14, marginTop: 16, cursor: 'pointer' }}>
          Saltear cardio
        </button>
      </div>
    )
  ...
  if (stage === 'done') {
    return (
      <div className="page fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100dvh', textAlign: 'center' }}>
        ...
        <button className="btn-primary" onClick={() => router.push('/home')} style={{ fontSize: 18, padding: '16px 40px' }}>
          VOLVER AL INICIO
        </button>
      </div>
    )
  ...
  return (
    <div className="page fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 40 }}>
      ...
      <div style={{ background: current.bg, border: `1px solid ${current.color}`, borderRadius: 12, padding: '8px 20px', marginBottom: 32 }}>
        <p style={{ fontSize: 13, color: current.color, fontWeight: 600, letterSpacing: '0.08em' }}>
      ...
      <div style={{ position: 'relative', width: 240, height: 240, marginBottom: 32 }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <p style={{ fontSize: 56, fontWeight: 700, color: current.color, fontFamily: 'var(--font-display)', lineHeight: 1 }}>
            {formatTime(timeLeft)}
          </p>
          {stage !== 'liss' && (
            <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 4 }}>
              Intervalo {intervalCount}
            </p>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 24, marginBottom: 40 }}>
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 2 }}>
            {stage === 'liss' ? 'LISS restante' : 'HIIT restante'}
          </p>
          <p style={{ fontSize: 20, fontWeight: 600, color: 'var(--text)' }}>
            {formatTime(stage === 'liss' ? lissTimeLeft : totalTimeLeft)}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 16 }}>
        <button
          ...
          style={{ width: 64, height: 64, borderRadius: '50%', border: '2px solid var(--border2)', background: 'var(--bg2)', color: 'var(--text)', fontSize: 24, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        >
          {isPaused ? '▶' : '⏸'}
        </button>
        <button
          onClick={nextStage}
          style={{ width: 64, height: 64, borderRadius: '50%', border: '2px solid var(--border2)', background: 'var(--bg2)', color: 'var(--muted)', fontSize: 13, cursor: 'pointer' }}
        >
          skip
        </button>
      </div>

      <button onClick={() => router.push('/home')} style={{ background: 'none', border: 'none', color: 'var(--border2)', fontSize: 13, marginTop: 24, cursor: 'pointer' }}>
        Salir del cardio
      </button>
